...
def init_tracker_if_needed(mode="individual", starting_credits=None):
    """
    Initialize sidebar credit display once per session.
    If starting_credits is provided, honor it; otherwise keep your default behavior.
    """
    if "plan" not in st.session_state:
        st.session_state["plan"] = mode
    else:
        st.session_state["plan"] = mode

    if "credits_remaining" not in st.session_state:
        if starting_credits is not None:
            st.session_state["credits_remaining"] = starting_credits
        else:
            # Your previous default; adjust if different
            st.session_state["credits_remaining"] = -1 if mode == "pro" else 15
...
def render_sidebar_badge():
    ...
    mode = st.session_state.get("plan", st.session_state.get("credit_mode", "individual")).lower()
    st.sidebar.markdown("### 💼 Plan")
    st.sidebar.write("**Pro (Unlimited)**" if mode == "pro" else "**Individual**")

    # Show credits only if not unlimited
    if mode != "pro":
        # Pull from session (already loaded from sheet in init_tracker_if_needed)
        ...
        # Progress bar uses your defined max
        st.sidebar.progress(
            min(1.0, max(0.0, rem / float(DEFAULT_INDIVIDUAL_CREDITS)))
        )
